...
function replace_subtree(func::UnaryOpNode, cur::Int, func2::Node, cur2::Int)
    ...
    subtree = child == cur2 ? func2 : replace_subtree(func.arg, child, func2, cur2)
...
function mh_resample_subtree_unbiased(prev_trace)
    ...
    func_new = replace_subtree(prev_trace.func, MAX_BRANCH-1, subtree, loc_delta)
...
function run_mcmc(prev_trace, iters::Int)
    ...
    for iter=1:iters
        ...
        if iter % 10000 == 0
            ...
            xs_model = evaluate_function(
                new_trace.func, new_trace.t0, new_trace.x0, length(new_trace.xs))
